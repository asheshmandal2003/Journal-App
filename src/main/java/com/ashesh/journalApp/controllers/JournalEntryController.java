...
public class JournalEntryController {
	...
	@PostMapping
	public ResponseEntity<ResponseData> saveJournalEntry(@RequestParam String username, @RequestBody JournalEntry newJournalEntry) {
		try {
			JournalEntry journalEntry = journalEntryService.saveJournalEntry(username, newJournalEntry);
			return new ResponseEntity<>(new ResponseData("Journal entry added!", journalEntry), HttpStatus.CREATED);
		}catch (Exception e){
	...
	public ResponseEntity<ResponseData> getAllJournalEntriesOfUser(@RequestParam String username){
		try {
			...
			return new ResponseEntity<>(new ResponseData("Journal entries fetched!", journalEntries), HttpStatus.OK);
		} catch (Exception e) {
			return new ResponseEntity<>(new ResponseData(e.getMessage(), null), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	public ResponseEntity<ResponseData> getJournalEntry(@PathVariable ObjectId id){
		try {
			...
			return new ResponseEntity<>(new ResponseData("Journal entry fetched!", journalEntry), HttpStatus.OK);
		}catch (Exception e){
			return new ResponseEntity<>(new ResponseData(e.getMessage(), null), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	@PutMapping("/{id}")
	public ResponseEntity<ResponseData> updateJournalEntry(@PathVariable ObjectId id, @RequestParam String username, @RequestBody JournalEntry newJournalEntry){
		try {
			JournalEntry journalEntry = journalEntryService.updateJournalEntry(id, newJournalEntry, username);
			return new ResponseEntity<>(new ResponseData("Journal entry updated!", journalEntry), HttpStatus.OK);
		}catch (Exception e){
			return new ResponseEntity<>(new ResponseData(e.getMessage(), null), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	@DeleteMapping("/{id}")
	public ResponseEntity<?> deleteJournalEntry(@PathVariable ObjectId id, @RequestParam String username){
		try {
			journalEntryService.deleteJournalEntry(id, username);
			return new ResponseEntity<>(new ResponseData("Journal entry deleted!", null), HttpStatus.OK);
		}catch (Exception e){
			return new ResponseEntity<>(new ResponseData(e.getMessage(), null), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
